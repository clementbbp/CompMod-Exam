...
Agents %>% 
  count(Group, sort = T)
Agents$Emptyx <- as.factor(Agents$Emptyx)
Agents$Emptyxx <- as.factor(Agents$Emptyxx)
   ...
   #Conditional statement for Bias
    ...
    for(i in 1:nrow(Agents)){
      ...
      if (RandNum < Latitude^Sharpness/(Distance^Sharpness+Latitude^Sharpness)){
        ...
        Agents$x[i] <- sum(Agents[i,seq(10, ncol(Agents), 2)])/((nonZero-7)/2) # -7 because there are always those 7 non memory columns which are nonZero.
