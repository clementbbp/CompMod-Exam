...
ABM1 <- function(Agents, Iterations, Plot_freq, Bias, Social_posting){
  ...
  for(t in 1:Iterations){
   ...
   #Conditional statement for Bias
    ...
    plot <- Out3 %>%
    ggplot(aes(x, y, color = Group)) + geom_point(aes(size = Share)) + xlim(-1, 1) + ylim(-1, 1) + theme(legend.position = "None") + ggtitle(paste("TickNo", 1)) +
  ...
  Agents$No_of_Shares - 1 # Subtracting one so it's the correct No_of_Shares
...
Lat_reject <- 0.8
Agents2 <- GenPop(nPop = 200, nMemo = 10, Lat_accept = 0.3, Sharpness = 4, nGroups = 4)
#Where do the agents start the simulation (default (0,0))
Agents2[,2] <- runif(nrow(Agents2), -1, 1) #Random x coordinates
Agents2[,3] <- runif(nrow(Agents2), -1, 1) #Random y coordinates

Out4 <- ABM1(Agents = Agents2, Iterations = 200, Plot_freq = 10, Bias = "Positive", Social_posting = TRUE)

Out <- ABM1(Agents = Agents2, Iterations = 200, Plot_freq = 10, Bias = "Positive", Social_posting = FALSE)
