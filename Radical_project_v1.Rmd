...
Agents2 <- GenPop(nPop = 100, nMemo = 10, Lat_accept = 0.3, Sharpness = 2, nGroups = 5)
  Agents %>% 
  count(Group, sort = T)
  ...
  for(t in 1:Iterations){
   ...
   #Conditional statement for Bias
    ...
    for(i in 1:nrow(Agents)){
      ...
      RandNum <- runif(1,0,1)
      if (RandNum < ((Latitude^Sharpness)/
                     (Distance_main^Sharpness+Latitude^Sharpness))){
...
Agents1 <- GenPop(nPop = 100, nMemo = 10, Lat_accept = 0.3, Sharpness = 5, nGroups = 5)
#Where do the agents start the simulation (default (0,0))
Agents1[,2] <- runif(nrow(Agents1), -1, 1) #Random x coordinates
Agents1[,3] <- runif(nrow(Agents1), -1, 1) #Random y coordinates
Out1 <- ABM1(Agents = Agents2, Iterations = 500, Plot_freq = 10, Bias = "None")
mean(Out1$x)
sd(Out1$x)
sd(Out1$y)
